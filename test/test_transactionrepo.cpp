...
private:
    std::shared_ptr<MockDateColumnAdapter> adapter;
};


test_TransactionRepo::test_TransactionRepo()
{
    adapter = std::make_shared<MockDateColumnAdapter>();
}
...
{
    auto provider = std::make_unique<MockTransactionProvider>();
    ...
    auto repo = std::make_unique<TransactionRepo>(adapter, std::move(provider));
...
//{
//    auto repo = std::make_unique<TransactionRepo>(adapter, provider);
//    provider->setTransactions({
